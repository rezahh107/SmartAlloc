...
$cap = 'manage_smartalloc';
if (!defined('SMARTALLOC_CAP')) {
    define('SMARTALLOC_CAP', $cap);
}
...
$_wp_transients=$_wp_actions=$_wp_filters=$_wp_cache=[];$t=$o=[];
if(!function_exists('get_transient')){function get_transient($k){global $t;return$t[$k]??false;}}
if(!function_exists('set_transient')){function set_transient($k,$v,$e){global $t;$t[$k]=$v;return true;}}
if(!function_exists('delete_transient')){function delete_transient($k){global $t;unset($t[$k]);return true;}}
...
if(!function_exists('wp_upload_dir')){function wp_upload_dir(){return ['basedir'=>sys_get_temp_dir(),'baseurl'=>'http://example.org/wp-content/uploads'];}}
if(!function_exists('wp_mkdir_p')){function wp_mkdir_p($d){return is_dir($d) ? true : mkdir($d,0777,true);} }
...
if(!function_exists('get_option')){function get_option($k,$d=false){global $o;$m=['siteurl'=>'http://example.org','home'=>'http://example.org','blogname'=>'Test Blog','admin_email'=>'admin@example.org'];return$o[$k]??$m[$k]??$d;}}
if(!function_exists('update_option')){function update_option($k,$v,$a=null){global $o;$o[$k]=$v;return true;}}
